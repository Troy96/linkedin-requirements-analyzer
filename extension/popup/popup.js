...
function setupEventListeners() {
  document
    ...
    .addEventListener("click", captureCurrentJob);
  document
    .getElementById("captureAllBtn")
    .addEventListener("click", captureAllJobs);
  document
  ...
  document
    ...
    .addEventListener("click", closeAnalysis);
  document
    .getElementById("fetchDescBtn")
    .addEventListener("click", fetchDescriptions);
  document.getElementById("stopFetchBtn").addEventListener("click", () => {
    sendMessage("stopFetching");
  });
  document
    .getElementById("saveSkillsBtn")
    .addEventListener("click", saveSkills);
  document
    .getElementById("generateStudyPlanBtn")
  ...
  try {
    ...
    if (
      ...
      pageTypeEl.textContent =
        "Navigate to a jobs page or saved jobs to capture";
      captureBtn.classList.add("hidden");
      captureAllBtn.classList.add("hidden");
      return;
    }
...
function renderJobList() {
  ...
  listElement.innerHTML = sortedJobs
    .map((job) => {
      const hasDescription =
        job.descriptionText && job.descriptionText.length > 0;
      const badge = !hasDescription
        ? '<span class="no-desc-badge" title="No description - click Open to capture full details">*</span>'
        : "";
...
async function captureCurrentJob() {
  ...
  try {
    ...
    if (results && results[0] && results[0].result) {
      ...
      showToast(
        "Could not extract job data. Make sure you're on a job detail page.",
        "error",
      );
    }
  } catch (error) {
...
async function captureAllJobs() {
  ...
  try {
    ...
    if (
      urlModified &&
      urlModified[0] &&
      urlModified[0].result &&
      urlModified[0].result.modified
    ) {
...
function checkAndClickNextPage() {
  ...
  const nextByTestId = document.querySelector(
  ...
  const prevByTestId = document.querySelector(
    '[data-testid="pagination-controls-previous-button-visible"]',
  );
  if (prevByTestId) {
    ...
    const result = await chrome.scripting.executeScript({
      ...
      func: (prevCount) => {
        ...
        return {
          ...
          ready:
            uniqueJobs.size > 0 &&
            (uniqueJobs.size !== prevCount || uniqueJobs.size > 0),
        };
      },
...
async function clearAllJobs() {
  ...
  if (
    !confirm(
      `Delete all ${currentJobs.length} captured jobs? This cannot be undone.`,
    )
  ) {
    return;
  }
...
function renderAnalysisResults(results) {
  ...
  const jobsWithDesc = currentJobs.filter(
    (j) => j.descriptionText && j.descriptionText.length > 0,
  ).length;
  ...
  if (Object.keys(results).length === 0) {
    ...
    if (jobsWithDesc === 0) {
      msg +=
        '<p style="margin-top: 8px; font-size: 12px; color: #666;">Jobs captured from list view don\'t include descriptions. Open each job and capture again for full analysis.</p>';
    } else {
      msg +=
        '<p style="margin-top: 8px; font-size: 12px; color: #666;">Try capturing more jobs with descriptions.</p>';
    }
...
function updateFetchSection() {
  ...
  const jobsWithoutDesc = currentJobs.filter(
    (j) => !j.descriptionText || j.descriptionText.length === 0,
  );
...
async function fetchDescriptions() {
  ...
  sendMessage("startFetching").then(async (result) => {
    ...
    if (result.success) {
      ...
      showToast(
        msg,
        result.errors > 0 && result.fetched === 0 ? "error" : "success",
      );
    } else {
...
function extractJobFromPage() {
  ...
  if (!title || title.length < 3) {
    const jobDetailEl = document.querySelector(
  ...
  for (const sel of locationSelectors) {
    ...
    for (const el of els) {
      ...
      if (
        text &&
        (text.includes(",") || /remote|hybrid|on-?site/i.test(text)) &&
        text.length < 100
      ) {
        location = text;
        break;
  ...
  if (!descriptionText) {
    const aboutEl = document.querySelector(
      '[class*="AboutTheJob"], [class*="about-the-job"]',
    );
    if (aboutEl) {
      ...
      if (!title) {
        ...
        for (const span of spans) {
          ...
          if (
            ...
            span.children[0].textContent.trim() !== text
          ) {
            continue;
          }
          if (
...
async function loadUserSkills() {
  try {
    const result = await chrome.storage.local.get("userSkills");
    if (result.userSkills) {
      document.getElementById("mySkillsInput").value = result.userSkills;
    }
  } catch (error) {
...
async function saveSkills() {
  const skills = document.getElementById("mySkillsInput").value;
  try {
...
async function generateStudyPlan() {
  if (currentJobs.length === 0) {
    showToast("No jobs captured yet", "error");
    return;
  }

  const skills = document.getElementById("mySkillsInput").value;
  if (!skills) {
    showToast("Please enter your skills first", "error");
    return;
  }
  ...
  try {
    ...
    downloadBlob(blob, `jobs-and-skills-${formatDateForFilename()}.json`);

    showToast("Data exported! Run study_plan.js next.", "success");

    // Show instructions in the analysis content area
    const container = document.getElementById("analysisContent");
    const resultsSection = document.getElementById("analysisResults");

    container.innerHTML = `
      <div class="study-plan-instructions">
        <h3>Next Steps</h3>
        <p>Your job data and skills have been exported.</p>
        <p>To generate your AI study plan, run:</p>
        <pre style="background: #f0f0f0; padding: 10px; border-radius: 4px; overflow-x: auto; font-size: 11px; margin: 10px 0;">node analyzer/study_plan.js jobs-and-skills-*.json</pre>
        <p style="margin-top: 10px; font-size: 12px; color: #666;">Make sure you have set your ANTHROPIC_API_KEY or OPENAI_API_KEY environment variable.</p>
      </div>
    `;
    resultsSection.classList.remove("hidden");
  } catch (error) {
